...
import fasofts.element.util.Constants.Companion.INVALID_PORT
...
class PersistentState(context: Context) : SimpleKrate(context), KoinComponent {
    ...
    // total blocklists set by the user for ElementDNS+ (server-side dns blocking)
    ...
    // whether apps subject to the ElementDNS VPN tunnel can bypass the tunnel on-demand
    // default: false for fdroid flavour
    var allowBypass by
        booleanPref("allow_bypass").withDefault<Boolean>(!Utilities.isFdroidFlavour())

    // user set among AppConfig.DnsType enum; RETHINK_REMOTE is default which is Element-DoH
    var dnsType by
        intPref("dns_type")
            .withDefault<Int>(
