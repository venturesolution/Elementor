...
class ConnectionMonitor(private val networkListener: NetworkListener) :
    ...
    private class NetworkRequestHandler(
        ...
        private suspend fun isReachable(host: String): Boolean {
            ...
            // event of the 'Element within Element' option being used, ICMP checks will fail.
            // Ideally, there is no need to perform ICMP checks. However, 'Element within
            // Element' is not supplied to this handler, these checks will be carried out but will
            // result in failure.
            try {
                val timeout = 500 // ms
                // https://developer.android.com/reference/android/net/Network#bindSocket(java.net.Socket)
