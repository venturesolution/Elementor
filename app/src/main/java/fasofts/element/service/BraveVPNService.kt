...
import fasofts.element.RethinkDnsApplication.Companion.DEBUG
import fasofts.element.data.AppConfig
import fasofts.element.data.ConnTrackerMetaData
import fasofts.element.data.ConnectionSummary
import fasofts.element.database.AppInfo
import fasofts.element.database.ConnectionTracker
import fasofts.element.database.RefreshDatabase
import fasofts.element.net.go.GoVpnAdapter
import fasofts.element.net.manager.ConnectionTracer
import fasofts.element.receiver.NotificationActionReceiver
import fasofts.element.scheduler.EnhancedBugReport
import fasofts.element.service.FirewallManager.NOTIF_CHANNEL_ID_FIREWALL_ALERTS
import fasofts.element.ui.HomeScreenActivity
import fasofts.element.ui.NotificationHandlerDialog
import fasofts.element.util.BackgroundAccessibilityService
import fasofts.element.util.Constants
import fasofts.element.util.Constants.Companion.INIT_TIME_MS
import fasofts.element.util.Constants.Companion.INVALID_UID
import fasofts.element.util.Constants.Companion.NOTIF_INTENT_EXTRA_ACCESSIBILITY_NAME
import fasofts.element.util.Constants.Companion.NOTIF_INTENT_EXTRA_ACCESSIBILITY_VALUE
import fasofts.element.util.Constants.Companion.PRIMARY_USER
import fasofts.element.util.Constants.Companion.UID_EVERYBODY
import fasofts.element.util.IPUtil
import fasofts.element.util.InternetProtocol
import fasofts.element.util.KnownPorts
import fasofts.element.util.NotificationActionType
import fasofts.element.util.OrbotHelper
import fasofts.element.util.Protocol
import fasofts.element.util.UIUtils.getAccentColor
import fasofts.element.util.Utilities
import fasofts.element.util.Utilities.isAtleastO
import fasofts.element.util.Utilities.isAtleastQ
import fasofts.element.util.Utilities.isAtleastS
import fasofts.element.util.Utilities.isAtleastU
import fasofts.element.util.Utilities.isMissingOrInvalidUid
import fasofts.element.util.Utilities.isNetworkSame
import fasofts.element.util.Utilities.isPlayStoreFlavour
import fasofts.element.util.Utilities.isUnspecifiedIp
...
class BraveVPNService :
    ...
    private suspend fun udpBlocked(uid: Int, protocol: Int, port: Int): Boolean {
        ...
        // https://github.com/@fernandoangeli/rethink-app/issues/492#issuecomment-1299090538
    ...
    private fun updateNotificationBuilder(): Notification {
        ...
        // 2. ElementDNS modes (dns & dns+firewall mode)
        // 3. No action button.
        ...
        // make it ongoing to prevent that. https://github.com/@fernandoangeli/rethink-app/issues/1136
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        ui {
            ...
            if (!isNewVpn) {
                ...
                io("startVpn") {
                    // refresh should happen before, otherwise the new vpn will not
    ...
    private fun addRoute4(b: Builder): Builder {
        if (persistentState.privateIps) {
            ...
            // prefixLength.
            // ref: github.com/@fernandoangeli/rethink-app/issues/26
    ...
    override fun flow(
        ...
        if (isRethink) {
            ...
            // if the connection is Element's uid and if the dup is false, then the connections
