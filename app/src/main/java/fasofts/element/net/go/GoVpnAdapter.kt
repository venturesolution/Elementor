 ...
 */
package fasofts.element.net.go
...
import fasofts.element.data.AppConfig.Companion.FALLBACK_DNS
import fasofts.element.data.AppConfig.TunnelOptions
import fasofts.element.database.DnsCryptRelayEndpoint
import fasofts.element.database.ProxyEndpoint
import fasofts.element.service.BraveVPNService
import fasofts.element.service.PersistentState
import fasofts.element.service.ProxyManager
import fasofts.element.service.ProxyManager.ID_WG_BASE
import fasofts.element.service.RethinkBlocklistManager
import fasofts.element.service.TcpProxyHelper
import fasofts.element.service.WireguardManager
import fasofts.element.util.Constants
import fasofts.element.util.Constants.Companion.MAX_ENDPOINT
import fasofts.element.util.Constants.Companion.ONDEVICE_BLOCKLIST_FILE_TAG
import fasofts.element.util.Constants.Companion.REMOTE_BLOCKLIST_DOWNLOAD_FOLDER_NAME
import fasofts.element.util.Constants.Companion.RETHINKDNS_DOMAIN
import fasofts.element.util.Constants.Companion.RETHINK_BASE_URL_MAX
import fasofts.element.util.Constants.Companion.RETHINK_BASE_URL_SKY
import fasofts.element.util.Constants.Companion.UNSPECIFIED_IP_IPV4
import fasofts.element.util.Constants.Companion.UNSPECIFIED_IP_IPV6
import fasofts.element.util.InternetProtocol
import fasofts.element.util.Utilities
import fasofts.element.util.Utilities.blocklistDir
import fasofts.element.util.Utilities.blocklistFile
import fasofts.element.util.Utilities.isPlayStoreFlavour
import fasofts.element.util.Utilities.showToastUiCentered
import fasofts.element.wireguard.Config
import intra.Intra
import intra.Tunnel
import java.net.URI
...
class GoVpnAdapter : KoinComponent {
    ...
    suspend fun addTransport() {
        ...
        // TODO: #321 As of now there is no block free transport for dns types other than Element.
    ...
    suspend fun addDnscryptRelay(relay: DnsCryptRelayEndpoint) {
        ...
        try {
            ...
            Logger.e(
                LOG_TAG_VPN,
                "connect-tunnel: dnscrypt relay failure: ${relay.dnsCryptRelayURL}",
                e
            )
    ...
    private suspend fun getProxyById(id: String): backend.Proxy? {
        return try {
    ...
    suspend fun refreshProxy(id: String) {
        if (!tunnel.isConnected) {
    ...
    suspend fun getProxyStats(id: String): backend.Stats? {
        return try {
    ...
    suspend fun setSystemDns(systemDns: List<String>) {
        ...
        // for Element within rethink mode, the system dns is system dns is always set to Ipn.Base
    ...
    suspend fun goBuildVersion(): String {
        return try {
            val version = Intra.build()
            Logger.i(LOG_TAG_VPN, "go build version: $version")
            version
        } catch (e: Exception) {
            Logger.e(LOG_TAG_VPN, "err go build version: ${e.message}", e)
            ""
        }
    }
